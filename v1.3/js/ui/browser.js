  ...
  return class extends fileSystemClass {
    ...
    async addLocalConfig(newConfig){
      ...
      try {
        ...
        let oldData = await configFileObject.read();
        data = await updateConfigFile(newConfig, oldData);
      } catch (error) {
        if (error instanceof FileNotFoundError){
          await this.fileSystem.waitOn(this.addLocalConfigFile.bind(this))();
        }
        data = await updateConfigFile({});
      }

      try{
        await this.fileSystem.write(this.constructor.localConfigPath, data);
      } catch (e) {
    ...
    async getConfig(){
      ...
      while (configFile === null && configPath.length > 0){
        ...
        try {
          ...
          localConfig = await parseConfigFile(data);
        } catch (error) {

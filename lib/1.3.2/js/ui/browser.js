...
class FileBrowser extends element_1.CustomElement {
    ...
    get path() {
        return this.currentDirectory.path.map((directory) => {
            return directory.name;
        });
    }
    set path(path) {
        this.logAndLoadWrapper(this.currentDirectory.root.getFile(path.slice(1))
            .then((newDirectory) => {
    ...
    handleDataTransfer(dataTransfer) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (pathText) {
                let path = JSON.parse(pathText).slice(1);
                ...
                moveConfirmDialog.addEventListener(dialog_1.ConfirmDialog.EVENT_CONFIRMED, () => {
                    ...
                    let moveFile = (path) => __awaiter(this, void 0, void 0, function* () {
                        let fileObject = yield this.rootDirectory.getFile(path);
                        yield fileObject.move(this.currentDirectory);
                    });
    ...
    execute(path) {
        ...
        new base_2.Process(null, this.rootDirectory, path, console, console);
    }

...
import { createWebSafeString, fileToArrayBuffer } from "../utils.js";
...
export class ContextMenu extends Dialog {
    ...
    createRenameButton(browser, selectedFile) {
        ...
        renameButton.onclick = () => {
            browser.logAndLoadWrapper((async () => {
                ...
                if (fileInput.files !== null) {
                    for (let file of fileInput.files) {
                        promises.push(fileToArrayBuffer(file).then((buffer) => {
                            ...
                            return browser.currentDirectory.addFile(buffer, webSafeName, file.type);
                        }));
                    }
                }
                this.visible = false;

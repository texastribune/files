...
import { Directory, FileNotFoundError } from "../files/base.js";
...
import { AbstractTableData, Row, Table } from "./elements/table";
...
export class FileSizeTableData extends AbstractTableData {
    constructor() {
        super(...arguments);
        this.file = null;
    }
...
export class FileTableData extends AbstractTableData {
    constructor() {
        super();
...
FileTableData.hoverImageClass = 'hover-image';
FileTableData.docIconClass = 'doc-and-folder-icon';
export class PathTableData extends AbstractTableData {
    get data() {
    ...
    set data(value) {
        this.textContent = value.map((segment) => {
...
export class FileBrowser extends CustomElement {
    constructor() {
        ...
        this.breadCrumbs.addEventListener(BreadCrumbs.EVENT_PATH_CHANGE, (event) => {
    ...
    set filePath(path) {
        this.clearMessages();
        this.logAndLoadWrapper(this.rootDirectory.getFile(path.slice(1))
            .then((newDirectory) => {
    ...
    getNewFileTableHeaderColumns() {
        let idColumn = document.createElement('text-data');
        let nameColumn = document.createElement('text-data');
        let sizeColumn = document.createElement('text-data');
        let lastModifiedColumn = document.createElement('text-data');
        let createdColumn = document.createElement('text-data');
        let typeColumn = document.createElement('text-data');
        let pathColumn = document.createElement('text-data');
    ...
    getNewFileTableHeader() {
        let header = document.createElement('table-header');
        header.appendChildren(this.getNewFileTableHeaderColumns());
        return header;
    }
    getNewFileTableRowColumns(rowData) {
        let idColumn = document.createElement('text-data');
        let nameColumn = document.createElement('file-data');
        let sizeColumn = document.createElement('size-data');
        let lastModifiedColumn = document.createElement('time-data');
        let createdColumn = document.createElement('time-data');
        let typeColumn = document.createElement('text-data');
    ...
    search(searchTerm) {
        this.clearMessages();
        if (searchTerm) {
            let currentSearch = this.logAndLoadWrapper(this.currentDirectory.search(searchTerm).then((searchResults) => {
...
// Class names
FileBrowser.actionsContainerId = 'file-actions-container';
FileBrowser.tableIconClass = 'icon';
FileBrowser.dropdownMenuIconClass = 'dropdown-icon';
FileBrowser.upLevelIconClass = 'uplevel-icon';
FileBrowser.leftSpaceClass = 'left-space';
FileBrowser.activeAjaxClass = 'ajax-active';
FileBrowser.messageContainerId = 'file-message-container';
FileBrowser.menuContainerId = 'file-menu-container';
FileBrowser.bodyContainerId = 'body-container';
FileBrowser.tableId = 'table';
FileBrowser.overlayId = 'overlay';
FileBrowser.buttonClass = 'button';
FileBrowser.disabledClass = 'disabled';
FileBrowser.gridItemClass = 'grid-item';
FileBrowser.carrotIconClass = 'carrot-icon';
...
FileBrowser.gridAttribute = 'grid';
/**
 * @event
 */
FileBrowser.EVENT_DIRECTORY_CHANGE = 'directory-change';
/**
 * @event
 */
FileBrowser.EVENT_FILES_CHANGE = 'files-change';
/**
 * @event
 */
